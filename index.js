...
function concatenateStrings(array, subString) {
  let i = array.toString();
  console.log(i.includes(subString))
...
function swapMinMax(array) {
  ...
  let [one , two, thre,four,five] = array;
  array = [one = max, two, thre,four,five = min];
  return array;
...
function getEvenNumbers(arr) {
  let empty = [];
  for (let i in arr) {
    arr[i] % 2 === 0 ? empty.push(arr[i]) : false
  };
  return empty
...
function replaceNumbers(arr) {
  let empty = [];
  for (let i in arr) {
    arr[i] <= 10 ? empty.push("Less than or equal to 10") : empty.push("Greater than 10");
  };
  return empty
  // Створюємо пустий масив для зберігання результату
  // Проходимо через кожен елемент вхідного масиву
